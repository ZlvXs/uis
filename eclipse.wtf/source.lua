...
local menu = game:GetObjects("rbxassetid://16657455028")[1] -- 16853049525
...
for _, obj in pairs(menu:GetDescendants()) do
   if obj.Name:lower():find('userlabel') then
       obj.Text = '<font color="rgb(255, 255, 255)">current user: </font><font color="rgb(189, 172, 255)">' .. game.Players.LocalPlayer.Name .. '</font>'
   end
   if obj.Name:lower():find('buildlabel') then
       obj.Text = '<font color="rgb(255, 255, 255)">build: </font><font color="rgb(189, 172, 255)">private</font>'
   end
end
...
function library:CreateLabel(Properties, IsHud)
   local _Instance = library:Create('TextLabel', {
      ...
      Font = Enum.Font.Code;
      TextColor3 = library.Colors.FontColor;
      TextSize = 16;
...
function library:Notify(Text, Time)
   ...
   local NotifyInner = library:Create('Frame', {
      BackgroundColor3 = library.Colors.MainColor;
   ...
   local Line = library:Create('Frame', {
      BackgroundColor3 = library.Colors.AccentColor;
      BorderSizePixel = 0;
...
function draggable(frame)
   ...
   local function onMouseInput(input)
      if not library.colorpicking then
         ...
         local newPosition = UDim2.new(startOffset.X.Scale, startOffset.X.Offset + delta.X, startOffset.Y.Scale, startOffset.Y.Offset + delta.Y)
...
InputService.InputEnded:Connect(function(key)
   if key.KeyCode == library.keybind then 
      ...
      for i,v in next, ibrary.toInvis do 
         v.Visible = false 
      end 
   end
end)
...
function library:addTab(name,image)
   ...
   function tab:createGroup(pos,groupname)
      ...
      function group:addToggle(args)
         ...
         if args.risky then 
            text.TextColor3 = state and Color3.fromRGB(255, 0, 0) or Color3.fromRGB(139, 0, 0) 
         end
         ...
         if state == true then
            ...
            if args.risky then 
               text.TextColor3 = Color3.fromRGB(139, 0, 0) 
            else 
               text.TextColor3 = Color3.fromRGB(144, 144, 144) 
            end
         end
      ...
      function group:addKeybind(args)
         if not args.flag then 
            return warn("⚠️ incorrect arguments ⚠️ - missing args on group:keybind") 
         end
      ...
      if config[i] then
         if v.type == "colorpicker" then 
            ...
            if v.type == "toggle" then 
               v.changeState(false) v.riskcfg(v.risky)
